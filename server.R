...
withProgress({
        ...
        }) },message = "Fetching data from openFDA API", min = 0,max = 10, value = 10)
...
withProgress(expr = {        
        ...
        },message = paste0("Parsing information about ",drug," from openFDA API"),min = 0,max = 10, value = 10)
...
withProgress(expr = {          
        ...
        },message = paste0("Calculating yearly PRR of ",event," for ",drug),min = 0,max = 10, value = 10)
         ...
         output$PPRplot<- renderPlotly({  
                 ...
                 isolate({
                        ...
                        withProgress(expr = {
                                ...
                                device.checkbox <-input$device.checkbox
                                ...
                                if(drug.FLAG){
                                        ...
                                        pl <- ggplot(df, aes(x,y)) + geom_blank() + xlim(0, 10) + ylim(0, 100)+geom_text(label = error.text,size = 4.5, color ="red")+
                                                labs(x="Date", y = "PRR")+
                                                theme(panel.background=element_rect(fill = "#ddbef7",colour = "black"),
                                                      panel.border=element_rect(colour = "black", fill = NA),
                                                      plot.title = element_text(hjust = 0.5, face = "bold",size = 15, colour = "navy"),
                                                      axis.title = element_text(size = 13, face = "bold", colour = "navy"),
                                                      axis.text = element_text(size = 13, face = "bold", colour = "navy"))
                                        
                                       
                                              
                                }else if(device.checkbox & !is.null(drug.device.data)) {
                                                        ...
                                                        pl <- ggplot(data = drug.data.temp , aes(x = end.year , y = PRR))+
                                                                geom_hline(yintercept = 1,size = 0.8, linetype = "dotted", color = "navy")+
                                                                geom_line(colour = "purple", size = 2)+
                                                                geom_point(colour = "navy", size = 4)+
                                                                labs(x="Date", y = "Drug adverse event PRR" ,title = paste0("Yearly PRR of adverse event ",gsub("\\+"," ",unique(drug.data.temp$event))," for drug ",unique(drug.data.temp$drug)))+
                                                                theme(panel.background=element_rect(fill = "#ddbef7",colour = "black"),
                                                                      panel.border=element_rect(colour = "black", fill = NA),
                                                                      plot.title = element_text(hjust = 0.5, face = "bold",size = 15, colour = "navy"),
                                                                      axis.title = element_text(size = 13, face = "bold", colour = "navy"),
                                        ...
                                        # pl <- ggplot(data = mtcars, aes(x = mpg, y= cyl))+
                                        #         geom_point()
                                        ...
                                        pl <- ggplot(data = drug.data.temp , aes(x = end.year , y = PRR))+
                                                ...
                                                theme(panel.background=element_rect(fill = "#ddbef7",colour = "black"),
                                                       panel.border=element_rect(colour = "black", fill = NA),
                                                       plot.title = element_text(hjust = 0.5, face = "bold",size = 15,colour = "navy"),
                                                       plot.subtitle = element_text(hjust = 0.5, face = "bold",size = 15, color ="red", colour = "navy"),
                                                       axis.title = element_text(size = 13, face = "bold", colour = "navy"),
                                                       axis.text = element_text(size = 13, face = "bold", colour = "navy"))
                             ...
                             },message = "Preparing the timeseries chart",min = 0,max = 10, value = 10) 
...
withProgress({                
   ...
   },message = "Calculating PMAs from openFDA API submitted to date.",min = 0,max = 10, value = 10)              
})
...
},message = "Calculating 510(k)s from openFDA API submitted to date.",min = 0,max = 10, value = 10)   
...
output$companionPMA <- renderPlotly({
        ...
        },message = "Calculating companion Dx PMAs from openFDA API submitted to date.",min = 0,max = 10, value = 10)              
})
